    ...
    <div style={{
      ...
      <div style={{ display: "grid", gridTemplateRows: open ? "1fr" : "0fr", transition: "grid-template-rows var(--dur-slow) var(--ease-standard)" }}>
        <div style={{ overflow: "hidden" }}>
          {/* Divergence assumée (cf. src/components/README.md) : le design
              system écrivait `padding` (raccourci) et `paddingTop` dans le même
              objet de style. React n'applique alors qu'une des deux règles et
              vide les trois autres côtés — le corrigé s'affichait collé au bord
              gauche, sans marge basse. Les quatre côtés sont donc explicites. */}
          <div style={{ paddingTop: "var(--sp-4)", paddingInline: "var(--sp-5)", paddingBottom: "var(--sp-5)", marginInlineStart: index ? "var(--sp-6)" : 0, font: "var(--type-small)", color: "var(--text-body)", borderTop: "1px solid var(--border-subtle)" }}>{children}</div>
        </div>
      </div>
    </div>
  );
